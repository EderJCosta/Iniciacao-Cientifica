...
<c:if test="${empresaLogada.tipo ne 'C'}">
    ...
    <div id="postar" class="postar"> 
        <div class="moldura-div">
            ...
            <form class="form-horizontal" action="<c:url value="/postagem/feed"/>" method="POST" onsubmit="return validacao_peso()">
                ...
                <div class="form-group semMargembott">
                    ...
                    <div class="col-sm-2 semMargembott">
                        ...
                        <c:if test="${empresaLogada.tipo eq 'F'}">
                            ...
                            <c:if test="${empresaLogada.tipo eq 'C'  or empresaLogada.tipo eq 'R'}">
                            <a href="<c:url value="#"/>" data-toggle="modal" data-target=".tipoADD"><span class="well well-sm glyphicon glyphicon-plus semMargembott" aria-hidden="true"></span></a>
                            </c:if>
...
<nav>
    <ul class="pager">
        <li class="previous <c:if test="${offset==0}">disabled</c:if> "><a  <c:if test="${offset>0}"> href="<c:url value="/postagem/feed?limit=${limit}&offset=${offset - limit}"/>"</c:if> ><span aria-hidden="true">&larr;</span> Voltar</a></li>
        <li class="next <c:if test="${qtde != limit || qtde == qtdeTotal}"> disabled </c:if>"><a <c:if test="${qtde == limit && qtde != qtdeTotal}"> href="<c:url value="/postagem/feed?limit=${limit}&offset=${offset + limit}"/>" </c:if>> Carregar<span aria-hidden="true">&rarr;</span></a></li>
        </ul>
    </nav>
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body">
                        <form <c:if test="${empresaLogada.tipo eq 'F'}"> action="<c:url value="/lixo/novo"/>" </c:if> <c:if test="${empresaLogada.tipo ne 'F'}"> action="<c:url value="/material/novo"/>" </c:if> method="POST">
                    <div class="form-group">
